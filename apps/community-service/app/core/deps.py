"""FastAPI dependencies: DB, current user, membership helpers."""
    ...
    *,
    ...
    if (
        ...
        raise HTTPException(
            ...
            detail="Moderator or owner required",
        )
...
async def require_group_owner(
    ...
    *,
    ...
    members = MemberRepository(conn)
    m = await members.get(group_id, user.uuid)
    if m is None or m.status != MemberStatus.approved or m.role != MemberRole.owner:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Group owner required"
        )
...
async def require_group_member(
    ...
    *,
    ...
    if m is None or m.status != MemberStatus.approved:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Group membership required"
        )
